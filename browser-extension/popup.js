...
chrome.storage.local.get('language').then(stored => {
...
languageSelect.addEventListener('change', async () => {
 ...
 render();
 try { await chrome.storage.local.set({language}); } catch { /* preference only */ }
});

select.addEventListener('change', render);

chrome.runtime.sendMessage({op: 'status'}).then(result => {
...
button.addEventListener('click', async () => {
 ...
 try { const result = await chrome.runtime.sendMessage({op: 'capture', accountID: select.value}); busy = false; setStatus(result.status); }
